...
class TimeSeriesAnalyzer:
    ...
    def advanced_seasonality_detection(self):
        ...
        # 1. TESTE HORÁRIO (por minuto)
        if temp_df['minute'].nunique() > 1 and len(temp_df) >= 60:
            minute_groups = [group['value'].values for name, group in temp_df.groupby('minute') if len(group) >= 2]
            
            if len(minute_groups) >= 3:
                try:
                    f_stat_minute, p_value_minute = f_oneway(*minute_groups)
                    ...
                    ssb_minute = ((temp_df_with_minute_mean['minute_mean'] - overall_mean) ** 2).sum()
                    sst = ((temp_df['value'] - overall_mean) ** 2).sum()
                    minute_variance_explained = (ssb_minute / sst) * 100 if sst > 0 else 0
                    
                    seasonality_tests['hourly'] = {
                        'has_pattern': p_value_minute < 0.05,
                        'p_value': p_value_minute,
                        'f_statistic': f_stat_minute,
        ...
        # 3. TESTE SEMANAL (por dia da semana)
        ...
        # 4. CLASSIFICAÇÃO (apenas predominante)
        hourly_var = seasonality_tests.get('hourly', {}).get('variance_explained', 0)
        ...
        hourly_sig = seasonality_tests.get('hourly', {}).get('has_pattern', False)
        daily_sig = seasonality_tests.get('daily', {}).get('has_pattern', False)
        weekly_sig = seasonality_tests.get('weekly', {}).get('has_pattern', False)
        
        STRONG_THRESHOLD = 15
        max_var = max(hourly_var, daily_var, weekly_var)
        
        if max_var < 5:
            main_seasonality = "LINEAR"
            season_type = "não sazonal"
        elif hourly_var == max_var and hourly_sig and hourly_var >= STRONG_THRESHOLD:
            main_seasonality = "SAZONAL_HORARIA"
            season_type = "horária"
        elif daily_var == max_var and daily_sig and daily_var >= STRONG_THRESHOLD:
            main_seasonality = "SAZONAL_DIARIA"
            season_type = "diária"
        elif weekly_var == max_var and weekly_sig and weekly_var >= STRONG_THRESHOLD:
            main_seasonality = "SAZONAL_SEMANAL"
            season_type = "semanal"
        else:
            if hourly_var == max_var and hourly_sig:
                main_seasonality = "SAZONAL_HORARIA"
                season_type = "horária"
            elif daily_var == max_var and daily_sig:
                main_seasonality = "SAZONAL_DIARIA"
                season_type = "diária"
            elif weekly_var == max_var and weekly_sig:
                main_seasonality = "SAZONAL_SEMANAL"
                season_type = "semanal"
            else:
                main_seasonality = "LINEAR"
    ...
    def get_detailed_patterns(self, temp_df):
        patterns = {}
        if temp_df.index.minute.nunique() > 1:
            patterns['minute'] = temp_df.groupby(temp_df.index.minute)['value'].agg(['mean', 'std', 'count'])
        if temp_df.index.hour.nunique() > 1:
            patterns['hourly'] = temp_df.groupby(temp_df.index.hour)['value'].agg(['mean', 'std', 'count'])
        if temp_df.index.dayofweek.nunique() > 1:
            patterns['weekly'] = temp_df.groupby(temp_df.index.dayofweek)['value'].agg(['mean', 'std', 'count'])
    ...
    def seasonal_decomposition(self):
        try:
            ...
            if seasonality_info['classification'] == 'SAZONAL_HORARIA':
                period = 60 if freq_seconds <= 60 else max(2, n // 10)
    ...
    def generate_plots(self):
        ...
        if hasattr(self, 'seasonality_results') and 'patterns' in self.seasonality_results:
            ...
            if available_patterns:
                ...
                titles = [k.replace('minute', 'Por Minuto').replace('hourly', 'Por Hora').replace('weekly', 'Por Dia da Semana') for k in available_patterns]
                ...
                for idx, pattern_type in enumerate(available_patterns, 1):
                    if pattern_type == 'minute':
                        ...
                        fig4.add_trace(go.Scatter(x=x_vals, y=y_vals, mode='lines+markers', 
                                     name='Média por Minuto', marker_color='lightblue'), row=1, col=idx)
                        ...
                        fig4.add_trace(go.Scatter(x=x_vals, y=y_vals, mode='lines+markers', 
                                     name='Média por Hora', marker_color='lightgreen'), row=1, col=idx)
                    elif pattern_type == 'weekly':
                        days = ['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom']
                        y_vals = [patterns['weekly'].loc[d, 'mean'] if d in patterns['weekly'].index else 0 for d in range(7)]
                        fig4.add_trace(go.Bar(x=days, y=y_vals, name='Média por Dia', marker_color='lightcoral'), row=1, col=idx)
        ...
        if jumps.get('jump_dates'):
            ...
            fig5.add_trace(go.Scatter(x=jump_dates, y=jump_values, mode='markers',
...
def show_seasonality_results(seasonality_results):
    ...
    color_map = {'SAZONAL_HORARIA': '🕐', 'SAZONAL_DIARIA': '🌅', 'SAZONAL_SEMANAL': '📅', 'LINEAR': '📈'}
...
def main():
    ...
    st.markdown("**Sistema para detectar padrões sazonais predominantes: Horário, Diário e Semanal**")
    st.markdown("---")
    ...
    if os.path.exists(series_folder):
        ...
        if csv_files:
            ...
            selected_file = st.sidebar.selectbox(
                ...
                csv_files,
                help="Selecione um arquivo CSV da pasta"
            )
        ...
        st.info("💡 Certifique-se de que a pasta existe e contém arquivos CSV com colunas 'Timestamp' e 'Valor'")
